  ...
  if (pDesc != NULL)
  {
    ...
    if (pDesc->remainingTimeMs != 0)
    {
      ...
      else
      {
        ...
        UINT8_TO_BSTREAM(pParams, MmdlGenDefaultTimeMsToTransTime(pDesc->transitionTimer.ticks *
                                                              WSF_MS_PER_TICK));
      }
...
static bool_t mmdlGenLevelSrProcessSet(const meshModelMsgRecvEvt_t *pMsg,  bool_t ackRequired)
{
  ...
  if (pDesc != NULL)
  {
    ...
    {
      /* Change state */
      mmdlGenLevelSrSetState(pMsg->elementId, state, transMs,
                             pMsg->pMessageParams[MMDL_GEN_LEVEL_SET_DELAY_IDX],
                             MMDL_STATE_UPDATED_BY_CL);
    }
    else
    {
      /* Change state */
      mmdlGenLevelSrSetState(pMsg->elementId, state, transMs, 0, MMDL_STATE_UPDATED_BY_CL);
    }
...
static bool_t mmdlGenLevelSrProcessMoveSet(const meshModelMsgRecvEvt_t *pMsg, bool_t ackRequired)
{
  ...
  if (pDesc != NULL)
  {
    ...
    if (pMsg->messageParamsLen == MMDL_GEN_LEVEL_SET_MAX_LEN)
    {
      ...
      /* Change state */
      mmdlGenLevelSrSetState(pMsg->elementId, targetState, movePeriod,
                             pMsg->pMessageParams[MMDL_GEN_LEVEL_SET_DELAY_IDX],
                             MMDL_STATE_UPDATED_BY_CL);
    }
    else
...
static bool_t mmdlGenLevelSrProcessDeltaSet(const meshModelMsgRecvEvt_t *pMsg, bool_t ackRequired)
{
  ...
  if (pDesc != NULL)
  {
    ...
    if (pMsg->messageParamsLen == MMDL_GEN_LEVEL_DELTA_SET_MAX_LEN)
    {
      ...
      /* Change state */
      mmdlGenLevelSrSetState(pMsg->elementId, targetState, transMs,
                             pMsg->pMessageParams[MMDL_GEN_LEVEL_DELTA_SET_DELAY_IDX],
                             MMDL_STATE_UPDATED_BY_CL);
    }
    else
    {
      ...
      /* Change state */
      mmdlGenLevelSrSetState(pMsg->elementId, targetState, transMs, 0, MMDL_STATE_UPDATED_BY_CL);
    }
    ...
    else if (pDesc->remainingTimeMs != 0)
    {
      ...
      */
      if (pDesc->isMoveSet)
      {
        ...
        {
          pDesc->pStoredStates[TARGET_STATE_IDX] = MMDL_GEN_LEVEL_MAX_SIGNED_LEVEL;
        }
        else if (target <= (int16_t)MMDL_GEN_LEVEL_MIN_SIGNED_LEVEL)
        {
          pDesc->pStoredStates[TARGET_STATE_IDX] = MMDL_GEN_LEVEL_MIN_SIGNED_LEVEL;
        }
      }

      /* Transition timeout. Move to Target State. */
      mmdlGenLevelSrSetState(elementId, pDesc->pStoredStates[TARGET_STATE_IDX], 0, 0,
                             pDesc->updateSource);
      ...
      {
        mmdlGenLevelSrSetState(elementId, pDesc->pStoredStates[TARGET_STATE_IDX], remainingTimeMs, 0,
                               pDesc->updateSource);
      }
    }
  }
