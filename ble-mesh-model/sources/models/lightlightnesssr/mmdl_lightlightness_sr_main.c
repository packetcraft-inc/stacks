...
static bool_t mmdlLightLightnessSrSet(const meshModelMsgRecvEvt_t *pMsg, bool_t ackRequired)
{
  ...
  if (pDesc != NULL)
  {
    ...
    if (pMsg->messageParamsLen == MMDL_LIGHT_LIGHTNESS_SET_MAX_LEN)
    {
      ...
      /* Change state */
      mmdlLightLightnessSrSetState(pMsg->elementId, state, transMs,
                             pMsg->pMessageParams[MMDL_SET_DELAY_IDX], MMDL_STATE_UPDATED_BY_CL);
      /* Set transition type */
      pDesc->transitionType = LIGHT_LIGHTNESS_ACTUAL_TRANSITION;
    }
    else
    {
      ...
      /* Change state */
      mmdlLightLightnessSrSetState(pMsg->elementId, state, transMs, 0, MMDL_STATE_UPDATED_BY_CL);
    }
...
static bool_t mmdlLightLightnessLinearSrSet(const meshModelMsgRecvEvt_t *pMsg, bool_t ackRequired)
{
  ...
  if (pDesc != NULL)
  {
    ...
    if (pMsg->messageParamsLen == MMDL_LIGHT_LIGHTNESS_LINEAR_SET_MAX_LEN)
    {
      ...
      /* Change state */
      mmdlLightLightnessLinearSrSetState(pMsg->elementId, state, transMs,
                              pMsg->pMessageParams[MMDL_SET_DELAY_IDX], MMDL_STATE_UPDATED_BY_CL);
      /* Set transition type */
      pDesc->transitionType = LIGHT_LIGHTNESS_LINEAR_TRANSITION;
    }
    else
    {
      ...
      /* Change state */
      mmdlLightLightnessLinearSrSetState(pMsg->elementId, state, transMs, 0,
...
static void mmdlLightLightnessSrHandleTmrCback(uint8_t elementId)
{
  ...
  if (pDesc != NULL)
  {
    ...
    else if (pDesc->remainingTimeMs != 0)
    {
      ...
      {
        mmdlLightLightnessLinearSrSetState(elementId, pDesc->pStoredStates[TARGET_STATE_IDX], 0, 0,
                                           pDesc->updateSource);
      }
    }
  }
