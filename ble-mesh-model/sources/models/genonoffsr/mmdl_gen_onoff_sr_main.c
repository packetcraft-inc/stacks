...
static bool_t mmdlGenOnOffSrProcessSet(const meshModelMsgRecvEvt_t *pMsg, bool_t ackRequired)
{
  ...
  if (pDesc != NULL)
  {
    ...
    if (pMsg->messageParamsLen == MMDL_GEN_ONOFF_SET_MAX_LEN)
    {
      mmdlGenOnOffSrSetState(pMsg->elementId, pMsg->pMessageParams[0], transMs,
                             pMsg->pMessageParams[MMDL_GEN_ONOFF_SET_DELAY_IDX],
                             MMDL_STATE_UPDATED_BY_CL);
    }
    else
    {
      mmdlGenOnOffSrSetState(pMsg->elementId, pMsg->pMessageParams[0], transMs, 0,
                             MMDL_STATE_UPDATED_BY_CL);
    }
    /* Save states */
