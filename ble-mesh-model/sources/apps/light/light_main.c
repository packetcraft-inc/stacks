 ...
 *  \param[in] state  button state. See ::PalBtnState_t.
 *
 *  \return    None.
 */
/*************************************************************************************************/
static void lightBtnCback(uint8_t btnId, PalBtnState_t state)
{
  /* Only alert application of button press and not release. */
  if ((btnId < LIGHT_BUTTON_MAX) && (state == PAL_BTN_STATE_DOWN))
  {
  ...
  switch(pEvt->param)
  {
    case MMDL_LIGHT_HSL_SR_STATE_UPDATE_EVENT:
      LIGHT_PRINT4("lighthsl_ind elemid%d h=%d s=%d l=%d " LIGHT_NEWLINE,
                   ...
                   ((mmdlLightHslSrStateUpdate_t *)pEvt)->state.hue,
                   ((mmdlLightHslSrStateUpdate_t *)pEvt)->state.saturation,
                   ((mmdlLightHslSrStateUpdate_t *)pEvt)->state.ltness);
      break;
