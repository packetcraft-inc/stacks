...
/* ED threshold for good CCA */
#define BB_154_DRV_CCA_ED_THRESHOLD       20

/* Bitcount checking */
/* Delay to sequence number to capture frame control field. */
#define BB_154_DRV_BCC_SEQ               (MAC_154_FRAME_CONTROL_LEN * 8)

/* Baseband driver resource interrupts */
...
/*
 *   PPI Channel 14 -  "Chan 14: TIMER[0].COMPARE[0] -> TXEN/RXEN"
 *
...
/*
 *   PPI Channel 15 -  "Chan 15: FRAMESTART -> CAPTURE[2]"
 *
...
#define BB_154_DRV_BACKOFF_FACTOR_US                (MAC_154_aUnitBackoffPeriod * PHY_154_SYMBOL_DURATION)
...
/* TODO: No radiodrv.c or cordio_154.h */
typedef struct
{
...
/*! \brief      Receive filter statistics. */
typedef struct
{
  uint32_t frameLenBad;             /*!< Frame length bad. */
  uint32_t rsvdFrameType;           /*!< Reserved frame type. */
  uint32_t bcnPanIdComp;            /*!< Beacon PAN ID compression. */
  uint32_t ackPanIdComp;            /*!< ACK PAN ID compression. */
  uint32_t dstRsvdAddr;             /*!< Destination reserved address. */
  uint32_t rsvdFrameVer;            /*!< Reserved frame version. */
  uint32_t bcnFrameVer;             /*!< Beacon frame version. */
  uint32_t datFrameVer;             /*!< Data frame version. */
  uint32_t srcRsvdAddr;             /*!< Source reserved address. */
  uint32_t bcnNoSrc;                /*!< Beacon no source. */
  uint32_t ackAddr;                 /*!< ACK address. */
  uint32_t panCompNoAddr;           /*!< PAN compression no address. */
  uint32_t dstPanId;                /*!< Destination PAN ID. */
  uint32_t dstAddrShort;            /*!< Destination address short. */
  uint32_t dstAddrLong;             /*!< Destination address long. */
  uint32_t bcnSrcPanId;             /*!< Beacon source PAN ID. */
  uint32_t cmdDataPanId;            /*!< Command/data PAN ID. */
  uint32_t panIdConflict;           /*!< PAN ID conflict. */
  uint32_t crc;                     /*!< CRC. */
} Cordio154DrvRxFiltStats_t;

/*! \brief    BBP states. */
typedef enum bb154DrvState
{
...
/*! \brief    BBP states. */
typedef enum bb154DrvRxHdrState
{
...
} bb154DrvRxHdrState_t;

/*! \brief    BBP control block. */
typedef struct bb154DrvCtrlBlk
{
  bb154DrvState_t       state;              /*!< BBP state. */
  bb154DrvRxHdrState_t  rxHdrState;         /*!< Rx header state */
  bool_t                rxAutoRestart;      /*!< TRUE if receive should automatically restart on frame failure. */
...
/*! \brief    BBP test control block. */
typedef struct bb154DrvTestCtrlBlk
{
  uint16_t            txCrcInit;          /*!< Transmit CRC initialization value. */
  uint32_t            errPattern;         /*!< Error pattern. */
  uint8_t             errPatternBit;      /*!< Error pattern bit. */
} bb154DrvTestCtrlBlk_t;
...
/*! \brief      Receive buffer descriptor. */ /* Note - must be packed so buffer immediately follows length */
typedef struct PAL_BB_154_PACKED Bb154DrvRxBufDesc
{
...
/* External function required. TODO Not in header files - must be linked with this driver. */
extern uint8_t *Bb154GetAddrsFromFrame(uint8_t *pFrame, uint16_t fctl, Mac154Addr_t *pSrcAddr, Mac154Addr_t *pDstAddr);
...
static bb154DrvCtrlBlk_t bb154DrvCb;

/*! \brief    Driver test control block. */
static bb154DrvTestCtrlBlk_t bb154DrvTestCb;

static RadioDrvTiming_t bb154DrvRadioTiming =
{
  .txOnLatency = 74,
  .rxOnLatency = 63,
  .txDataPathLatency = 8,
...
static Cordio154DrvStats_t bb154DrvStats;

/*! \brief    Receive filter statistics. */
static Cordio154DrvRxFiltStats_t bb154DrvRxFiltStats;
 ...
 */
/*************************************************************************************************/
void Cordio154DrvGetStats(Cordio154DrvStats_t *pStats)
{
  memcpy(pStats, &bb154DrvStats, sizeof(*pStats));
}

/*************************************************************************************************/
/*!
 *  \brief  Get receive filter statistics.
 *
 *  \param  pStats          Storage for statistics.
 *
 *  \return None.
 */
/*************************************************************************************************/
void Cordio154DrvGetRxFiltStats(Cordio154DrvRxFiltStats_t *pStats)
{
  memcpy(pStats, &bb154DrvRxFiltStats, sizeof(*pStats));
}

/*************************************************************************************************/
/*!
 *  \brief      Set receive auto-restart.
 *
 *  \param      rxAutoRestart   TRUE if receive should automatically restart on frame failure.
 *
 *  \return     None.
 */
/*************************************************************************************************/
void Cordio154DrvSetRxAutoRestart(bool_t rxAutoRestart)
{
  bb154DrvCb.rxAutoRestart = rxAutoRestart;
}

/*************************************************************************************************/
/*!
 *  \brief      Set transmit CRC initialization.
 *
 *  \param      txCrcInit   Transmit CRC initialization.
 *  \param      errPattern  Error pattern.
 *
 *  \return     None.
 */
/*************************************************************************************************/
void Cordio154DrvTestSetTxCrcInit(uint16_t txCrcInit, uint32_t errPattern)
{
  bb154DrvTestCb.txCrcInit     = txCrcInit;
  bb154DrvTestCb.errPattern    = errPattern;
  bb154DrvTestCb.errPatternBit = 0;
}
...
static bool_t bb154DrvStartRadioTask(uint32_t due, bool_t now, nrf_radio_task_t radioTask)
{
  ...
  else
  {
    ...
    if (!BB_154_DRV_TIME_IN_FUTURE(due - BB_154_DRV_SETUP_TIME, PalBbGetCurrentTime(USE_RTC_BB_CLK)))
    {
...
static void bb154DrvClearAllEvents(void)
{
  ...
  while (!nrf_radio_event_get(NRF_RADIO_EVENT_DISABLED)) ;
...
static void bb154DrvOff(void)
{
 ...
 /* Disable all peripheral shortcuts which might be enabled */
  nrf_radio_shorts_disable(NRF_RADIO_SHORT_END_DISABLE_MASK |
                           ...
                           NRF_RADIO_SHORT_READY_CCASTART_MASK |
...
static void bb154DrvRxData(uint32_t due, bool_t now, uint32_t timeout)
{
  ...
  if (timeout > 0)
  {
    if (now)
    {
      ...
      due = PalBbGetCurrentTime(USE_RTC_BB_CLK);
    }
  ...
  {
    bb154DrvCb.rx.pBuf = WsfQueueDeq(&bb154DrvCb.rx.bufQ);
  }

  /* Set packet pointer to supplied buffer. Length obtained from first field */
  nrf_radio_packet_ptr_set(bb154DrvCb.rx.pBuf);
...
static void bb154DrvNextRx(bool_t rxAck)
{
  if ((bb154DrvCb.rx.pBuf != NULL) || !WsfQueueEmpty(&bb154DrvCb.rx.bufQ))
  {
    uint32_t timeout;
    bb154DrvState_t nextState;

    if (rxAck)
    {
      timeout =  PAL_BB_154_SYMB_TO_US(MAC_154_RX_ACK_TIMEOUT_SYMB);
...
static void bb154DrvTxDataNoCca(uint32_t due, bool_t now)
{
  ...
  if (bb154DrvCb.state == BB_154_DRV_STATE_TX)
  {
    ...
    nrf_radio_packet_ptr_set(&bb154DrvCb.tx.pDesc->len);
  }
  else
  {
    /* Set packet pointer to the length field of ack packet */
    nrf_radio_packet_ptr_set(bb154DrvCb.ack);
  }
...
static void bb154DrvMacCca(uint32_t due, bool_t now)
{
  ...
  if (bb154DrvCb.tx.be > 0)
  {
    ...
    PrandGen((uint8_t *)&backoffSymb, sizeof(backoffSymb));
  ...
  nrf_radio_shorts_enable(NRF_RADIO_SHORT_READY_CCASTART_MASK);
...
static void bb154DrvTxData(uint32_t due, bool_t now)
{
  ...
  if (!pPib->disableCCA && ((bb154DrvCb.op.flags & PAL_BB_154_FLAG_DIS_CCA) == 0))
  {
    /* Update event state. */
    bb154DrvCb.state = BB_154_DRV_STATE_TX_CCA;
    bb154DrvCb.tx.nb = 0;            /* reset NB */
    bb154DrvCb.tx.be = pPib->minBE;  /* reset BE */
...
static uint8_t bb154DrvEvHGenlRxTOTmrExp(void)
{
  ...
  BB_ASSERT((radioState == NRF_RADIO_STATE_RX) || (radioState == NRF_RADIO_STATE_RX_RU));
...
static uint8_t bb154DrvEvHRxHdBcMatch(void)
{
  ...
  switch (nrf_radio_state_get())
  {
    case NRF_RADIO_STATE_RX:
    case NRF_RADIO_STATE_RX_IDLE:
    case NRF_RADIO_STATE_RX_DISABLE: /* A lot of states due to shortcuts */
    case NRF_RADIO_STATE_DISABLED:
    case NRF_RADIO_STATE_TX_RU:
      if (pPib->promiscuousMode)
      {
      ...
      else
      {
        ...
        BYTES_TO_UINT16(fctl, pRxFrame);

        switch (bb154DrvCb.rxHdrState)
        {
          case BB_154_DRV_HDR_STATE_SEQ:
            if (MAC_154_FC_SECURITY_ENABLED(fctl))
            {
          ...
          case BB_154_DRV_HDR_STATE_PAYLOAD:
            /* At payload. Check addresses. */
            {
              ...
              restart = TRUE;
              if ((dstAddr.addrMode == MAC_154_ADDR_MODE_NONE) && (pPib->deviceType == MAC_154_DEV_TYPE_PAN_COORD))
              {
              ...
              else
              {
                ...
                BYTES_TO_UINT16(panId, dstAddr.panId);

                if ((panId == bb154DrvCb.rxAddr.panId) ||
                    (panId == MAC_154_BROADCAST_PANID))
                {
                    ...
                    case MAC_154_ADDR_MODE_SHORT:
                      {
                        ...
                        BYTES_TO_UINT16(shtAddr, dstAddr.addr);
                        if ((shtAddr == bb154DrvCb.rxAddr.shortAddr) ||
                            (shtAddr == MAC_154_BROADCAST_ADDR))
                        {
                    ...
                    case MAC_154_ADDR_MODE_EXTENDED:
                      {
                        ...
                        BYTES_TO_UINT64(extAddr, dstAddr.addr);
              ...
              if (!restart)
              {
                ...
                BYTES_TO_UINT64(addr, srcAddr.addr)
                /* Call frame pending check callback */
                if (((fctl & MAC_154_FC_FRAME_TYPE_FP_TEST) == MAC_154_FC_FRAME_TYPE_FP_TEST) &&
    ...
    case NRF_RADIO_STATE_TX_RU:
      {
        ...
        /* Get Frame Control field. */
        BYTES_TO_UINT16(frameCtrl, &bb154DrvCb.rx.pBuf[BB_154_DRV_RX_FRAME_CTRL_OFFSET]);
        if (MAC_154_FC_FRAME_TYPE(frameCtrl) != MAC_154_FRAME_TYPE_ACKNOWLEDGMENT)
        {
          /* Skip and restart Rx */
          bb154DrvNextRx(FALSE);
        }
...
static uint8_t bb154DrvEvHRxFmEnd(void)
{
  ...
  if (nrf_radio_crc_status_get() == NRF_RADIO_CRC_STATUS_OK)
  {
    ...
    uint16_t len        = bb154DrvCb.rx.pBuf[0] - MAC_154_FCS_LEN; /* Remove length of CRC */
    int8_t rssi;
    ...
    if (!pPib->promiscuousMode &&
        ...
        ((fctl & MAC_154_FC_FRAME_TYPE_ACK_TEST) == MAC_154_FC_FRAME_TYPE_ACK_TEST))
    {
      /* Rx requested an ack. so send it */
      //uint32_t due = ts + PAL_BB_154_SYMB_TO_US((((len + MAC_154_FCS_LEN) * PHY_154_SYMBOLS_PER_OCTET) + PHY_154_aTurnaroundTime));
      //uint32_t due = ts + PAL_BB_154_SYMB_TO_US((((len + MAC_154_FCS_LEN) * PHY_154_SYMBOLS_PER_OCTET) + PHY_154_aTurnaroundTime)) + 200;
      uint32_t due = PalBbGetCurrentTime(USE_RTC_BB_CLK) + PAL_BB_154_SYMB_TO_US(PHY_154_aTurnaroundTime);

      bb154DrvCb.state = BB_154_DRV_STATE_TX_ACK;

      if (!((MAC_154_FC_FRAME_TYPE(fctl) == MAC_154_FRAME_TYPE_MAC_COMMAND) &&
            (*PalBb154GetPayloadPtr(pRxFrame, fctl) == MAC_154_CMD_FRAME_TYPE_DATA_REQ)))
      {
...
static uint8_t bb154DrvEvHTxFmEnd(void)
{
  ...
  if (bb154DrvCb.op.flags & PAL_BB_154_FLAG_TX_AUTO_RX_ACK)
  {
    ...
    ackReqd = (((pTxFrame[0] & MAC_154_FC_ACK_REQUEST_MASK) != 0) &&
               (((pTxFrame[0] & MAC_154_FC_FRAME_TYPE_MASK) == MAC_154_FRAME_TYPE_DATA) ||
                ((pTxFrame[0] & MAC_154_FC_FRAME_TYPE_MASK) == MAC_154_FRAME_TYPE_MAC_COMMAND) ||
                ((pTxFrame[0] & MAC_154_FC_FRAME_TYPE_MASK) == MAC_154_FRAME_TYPE_ILLEGAL4)));
#else
    ackReqd = (((pTxFrame[0] & MAC_154_FC_ACK_REQUEST_MASK) != 0) &&
               (((pTxFrame[0] & MAC_154_FC_FRAME_TYPE_MASK) == MAC_154_FRAME_TYPE_DATA) ||
                ((pTxFrame[0] & MAC_154_FC_FRAME_TYPE_MASK) == MAC_154_FRAME_TYPE_MAC_COMMAND)));
...
static uint8_t bb154DrvEvHEnDtEdEnd(void)
{
  //uint8_t ed = (uint8_t)nrf_radio_ed_sample_get();
...
static uint8_t bb154DrvEvHTxCCIdle(void)
{
  ...
  /* Disable shortcut */
  nrf_radio_shorts_disable(NRF_RADIO_SHORT_READY_CCASTART_MASK);
...
static uint8_t bb154DrvEvHTxCCBusy(void)
{
  ...
  if (++bb154DrvCb.tx.nb == pPib->maxCSMABackoffs)
  {
  ...
  {
    /* Retry CCA */
    if (++bb154DrvCb.tx.be > pPib->maxBE)
    {
      /* Bump at maxBE */
      bb154DrvCb.tx.be = pPib->maxBE;
    }
...
void PalBb154Init(void)
{
  ...
  memset(&bb154DrvTestCb,      0, sizeof(bb154DrvTestCb));
  memset(&bb154DrvStats,       0, sizeof(bb154DrvStats));
  memset(&bb154DrvRxFiltStats, 0, sizeof(bb154DrvRxFiltStats));
...
void PalBb154Enable(void)
{
  ...
  nrf_radio_config_max_length_set(PHY_154_aMaxPHYPacketSize);

  /* CRC configuration */
  nrf_radio_crc_length_set(BB_154_DRV_CRC_LENGTH);
  nrf_radio_crc_includes_address_set(NRF_RADIO_CRC_INCLUDES_ADDR_IEEE802154);
#ifdef ADDITIONAL_TESTER_FEATURES
  if (Mac154GetPIB()->vsCRCOverride)
  {
    nrf_radio_crc_polynominal_set(Mac154GetPIB()->vsCRCOverride);
  }
  else
  {
    nrf_radio_crc_polynominal_set(BB_154_DRV_CRC_POLYNOMIAL);
  }
#else
  nrf_radio_crc_polynominal_set(BB_154_DRV_CRC_POLYNOMIAL);
#endif
  ...
  /* CCA configuration */
  nrf_radio_cca_mode_set(NRF_RADIO_CCA_MODE_ED);
  nrf_radio_cca_ed_threshold_set(BB_154_DRV_CCA_ED_THRESHOLD);
...
void PalBb154FlushPIB(void)
{
  ...
  if (!pPib->promiscuousMode)
  {
    ...
    bb154DrvCb.rxAddr.panId = pPib->panId;
    bb154DrvCb.rxAddr.shortAddr = pPib->shortAddr;
    bb154DrvCb.rxAddr.extAddr = pPib->extAddr;
  }
}
...
void PalBb154Tx(PalBb154TxBufDesc_t *pDesc, uint8_t cnt, uint32_t due, bool_t now)
{
  ...
  /* Set retry counter */
  bb154DrvCb.tx.retry = pPib->maxFrameRetries;
...
void PalBb154ClearRxBufs(void)
{
  ...
  if (bb154DrvCb.rx.pBuf != NULL)
  {
    WsfBufFree(bb154DrvCb.rx.pBuf);
    bb154DrvCb.rx.pBuf = NULL;
  }

  /* Free any queued receive buffers */
  while ((pRxBuf = WsfQueueDeq(&bb154DrvCb.rx.bufQ)) != NULL)
  {
    WsfBufFree(pRxBuf);
  }
...
uint8_t PalBb154ReclaimRxFrame(uint8_t *pRxFrame)
{
  if (pRxFrame != NULL)
  {
    WsfQueueEnq(&bb154DrvCb.rx.bufQ, BB_154_DRV_RX_BUF_PTR_FROM_FRAME(pRxFrame));
    bb154DrvCb.rx.bufCount++;
  }
...
void PalBb154BuildRxBufQueue(uint16_t len, uint8_t num)
{
  ...
  {
    if ((pRxBuf = WsfBufAlloc(len)) == NULL)
    {
      break;
    }
    WsfQueueEnq(&bb154DrvCb.rx.bufQ, pRxBuf);
    bb154DrvCb.rx.bufCount++;
  }
}
...
uint8_t *PalBb154GetPayloadPtr(uint8_t *pFrame, uint16_t fctl)
{
  ...
  offset = amLUT[MAC_154_FC_DST_ADDR_MODE(fctl)] + amLUT[MAC_154_FC_SRC_ADDR_MODE(fctl)];
...
void PalBb154Rx(uint32_t due, bool_t now, uint32_t timeout)
{
  ...
  BB_ASSERT(!WsfQueueEmpty(&bb154DrvCb.rx.bufQ));
...
void PalBb154Ed(uint32_t due, bool_t now)
{
  ...
  /* 60 is half an aBaseSuperframeDuration time and matches Cordio radio */
...
static void Bb154DrvRadioIRQHandler(void)
{
  ...
  for (i = 0; i < BB_154_DRV_MAX_RADIO_EVENTS; i++)
  {
    ...
    if (nrf_radio_event_get(eventToCheck))
    {
      /* Clear event */
